...
If  a |BMS-Slave| with a hardware version ``2.x.x`` (green color) is used, section :ref:`hw_slave_12cell_v2_x` must be used for pin assignments and cabling instructions. For slave hardware versions ``1.x.x`` (red color), section :ref:`hw_slave_12cell_v1_x` must be used.
