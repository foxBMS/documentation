...
In this section, it is shown how to set up an Eclipse workspace to work with the |foxBMS| sources. Within the workspace, it is possible to flash the compiled sources on the |Master|.
...
.. note::
    This tutorial uses Unix-style delimiting character (``/``) in paths. Under Windows, Windows-style delimiting character (``\``) in paths must be used.
...
..  note::
    ...
    -   Variable 1: |setup_path|

        If the setup dir is |setup_dir|, the path has to be set to ``C:\Users\username\Documents`` and ``C:\Users\username\Documents\foxBMS-setup`` has to be used instead of |setup_path|.

    |

    -   Varibale 2: |foxconda_path|

            -   If |foxconda_name| is installed into the default and preferred installation directory |foxconda_default_windows_path| |foxconda_default_windows_path| has to be used instead of |foxconda_path|.

            -   If |foxconda_name| is installed into |foxconda_default_windows_path| some steps of these instructions can be skipped. These steps are indicated.
...
-   |foxbms| source files are downloaded to |setup_path|. Getting the source files is described in section ":ref:`getting_started_build`".

-   `Eclipse CDT <https://www.eclipse.org/cdt/>`_ , in version Oxygen.1.Release (4.7.1a). Eclipse CDT Oxygen.1.Release (4.7.1a) can be downloaded form `eclipse.org <https://www.eclipse.org/downloads/packages/eclipse-ide-cc-developers/oxygenr>`_. After downloading the .zip file and extracting it, ``eclipse.exe`` must be started to launch ``Eclipse CDT``.

-   The following plugins must be installed into ``Eclipse CDT``. ``PyDev`` (`PyDev project website <https://www.pydev.org/>`_) is mandatory, while ``LiClipseText`` (`LiClipseText project website <http://www.liclipse.com/>`_) is optional. However it is recommended to install both plugins. The plugins can be  installed from the ``Eclipse Marketplace``.
...
#.  Start ``Eclipse`` and click ``File`` -> ``Switch Workspace`` -> ``other`` and configure ``path/to/foxBMS-setup/.ws`` as workspace, see :numref:`Fig. %s <switch_workspace>`.
...
|OK| |foxconda| is now successfully selected as Python interpreter. |OK|
...
..  note:: 
        If a |foxconda| version greater than 0.5 is used and |foxconda_name| was installed in the default installation path (|foxconda_default_windows_path|), this section can be skipped. 
...
#.  Goto ``C/C++ Build`` -> ``Environment``.
    ...
    ..  _eclipse_project_properties_c_c++_environment.png:
    ..  figure:: eclipse-project-properties-c-c++-environment.png

        ``C/C++ Build`` project properties

        #. Click |ECLIPSEADD| and

            #.  use ``FOXCONDA`` for ``Name``

            #.  use
                ``path/to/foxconda;``
                ``path/to/foxconda/bin;``
                ``path/to/foxconda/Scripts;``
                ``path/to/foxconda/Library/bin;``
                ``path/to/foxconda/Lib;``
                for ``Value``.
        ...
        #. Select the ``PATH`` entry and click |ECLIPSEEDIT|
            ...
            #.  **Pre**\ pend ``${FOXCONDA};`` to the already specified values in ``Value``
...
#.  Repeat these steps for the ``Documentation`` and the ``Secondary`` project.
...
#.  Select the ``Primary`` project and click right. Goto ``C/C++ General`` -> ``Paths and Symbols`` and switch to Tab ``Includes``.

#.  Click |ECLIPSEADD| for ``Assembly``,
    ``GNU C`` and ``GNU C++`` and configure the following two paths as ``includes``

    #. For |foxconda_name| version **greater than 0.5** the following applies:

        #. ``path/to/foxconda/Library/arm-none-eabi/include``
    
        #. ``path/to/foxconda/Library/lib/gcc/arm-none-eabi/6.3.1/include``
        
    #. For |foxconda_name| version **0.5 or less** the following applies:
    
        #. ``path/to/foxconda/arm-none-eabi/include``

        #. ``path/to/foxconda/Lib/gcc/arm-none-eabi/4.9.3/include`` 
...
#.  ``Documentation`` project
    ...
    #.  Select the ``Documentation`` project and click on the dropdown menu of the |ECLIPSEHAMMER| button and select ``1 sphinx`` to build the general |foxbms| documentation.

        ..  _test_general_documentation.png:
        ..  figure:: test-general-documentation.png

            Generating the |foxbms| documentation

        The documentation generation was successful, if the Eclipse console puts the following line:

        ..  code-block:: console

            Waf: Leaving directory `path/to/foxBMS-setup/build/sphinx'
            'sphinx' finished successfully (2.329s)
            INFO:root:Success: Process return code 0

    #. The ``Documentation`` project is now successfully tested.

#.  ``Primary`` project

    #.  Select the ``Primary`` project and click on the dropdown menu of the |ECLIPSEHAMMER| button and select ``1 build`` to build the |foxbms| binaries of the primary mcu.

        ..  _test_primary_binaries.png:
        ..  figure:: test-primary-binaries.png

            Building the primary |foxbms| binaries

        The binaries generation was successful, if the Eclipse console puts the following line at the bottom:

        ..  code-block:: console

            Waf: Leaving directory `path/to/foxBMS-setup/build/primary'
            'build' finished successfully (1.340s)
            INFO:root:Success: Process return code 0

    #.  Select the ``Primary`` project and click on the dropdown menu of the |ECLIPSEHAMMER| button and select ``3 doxygen`` to build the |foxbms| source code documentation of the primary mcu.

        ..  _test_primary_doxygen.png:
        ..  figure:: test-primary-doxygen.png

            Building the primary |foxbms| binaries

        There might be warnings (e.g., about not documented parameters). This can be ignored and the doxygen documentation generation was successful as long as the last line of the Eclipse console states:

        ..  code-block:: console

            INFO:root:Success: Process return code 0

    #.  Select the ``Primary`` project and click on the dropdown menu of the |ECLIPSEHAMMER| button and select ``2 clean`` to clean the |foxbms| binaries and the doxygen documentation of the primary mcu.

        ..  _test_primary_clean.png:
        ..  figure:: test-primary-clean.png

            Cleaning the primary |foxbms| binaries and doxygen documentation

        Cleaning was successful if the Eclipse console puts the following line at the bottom:

        ..  code-block:: console

            'clean' finished successfully (0.050s)
            INFO:root:Success: Process return code 0
...
.. |ECLIPSEIMPORT|          image:: eclipse-import.png
...
.. |setup_dir|                      replace:: ``foxbms-setup``
.. |setup_path|                     replace:: ``path/to/foxbms-setup``
.. |foxconda_name|                  replace:: ``foxconda``
.. |foxconda_path|                  replace:: ``path/to/foxconda``
.. |foxconda_pythonexe_path|        replace:: ``path/to/foxconda/python.exe``
.. |foxconda_default_windows_path|  replace:: ``C:\foxconda``
...
In order to program the primary MCU, the mini USB jack indicated as ``Prim. USB`` in :numref:`fig. %s <foxbmsfront_quickstart_flash>` must be used to connect the |master| to a PC. The |master| can be connected to a PC immediately. When connecting |master| for the first time, the required drivers will install automatically. 
...
The same procedure must be made for secondary: the mini USB jack indicated as ``Sec. USB`` in :numref:`fig. %s <foxbmsfront_quickstart_flash>` must be used to connect the |master| to a PC. As for the primary MCU, the PC must be connected to the internet before the connection is made with the |master| for the first time.

Flash the Compiled Source for the Primary MCU
---------------------------------------------

In the chapter :ref:`getting_started_build`, the |foxbms| sources have been compiled and the generated binary is ready to be flashed.
...
The device must be connected to the computer with the USB cable. The |MCU0| USB-connector must be used. The ``Primary`` project must be select. Clicking on the dropdown menu of the |ECLIPSEHAMMER| button and selecting ``5 flash`` allows flashing the |foxbms| binaries of the primary mcu.
...
The same procedure as explained above must be followed for the |MCU1|. The difference is that the |MCU1| USB-connector must be used. The ``Secondary`` project must be selected before clicking on the dropdown menu of the |ECLIPSEHAMMER| button. ``5 flash`` must then be selected to flash the |foxbms| binaries of the secondary mcu.
...
The next step shown in :ref:`getting_started_cabling` is to connect a |slave| to perform voltage and temperature measurement.
